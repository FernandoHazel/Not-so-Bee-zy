...
public class OffScreenIndicator : MonoBehaviour
{
    ...
    [SerializeField] GameObject pollensCanvas;
    ...
    private void HandleTargetStateChanged(Target target, bool active)
    {
        ...
        else
        {
            ...
            pollensCanvas.GetComponent<PollensCanvas>().DisplayPollen();
    ...
    /// <returns></returns>
    private Indicator1 GetIndicator(ref Indicator1 indicator, IndicatorType type)
    {
        if(indicator != null)
        {
            if(indicator.Type != type)
            {
                ...
                indicator = type == IndicatorType.BOX ? BoxObjectPool.current.GetPooledObject() : ArrowObjectPool1.current.GetPooledObject();
